...
def generate_qwen(prompt, max_new_tokens=300, temperature=0.7, top_p=0.9):
...
class DebateProcessor:
    ...
    def _run_debate_with_retry(
        ...
        """Run debate locally with Qwen (no API or key rotation)."""
        ...
        # Generate attacker, defender, and judge responses
        from debate_logic import RedTeamingDebate
        debate = RedTeamingDebate(
            attacker_model="local_qwen",
            defender_model="local_qwen",
            judge_model="local_qwen"
        )
...
def main():
    ...
    # Process dataset
    processor = DebateProcessor(pipeline, config)
